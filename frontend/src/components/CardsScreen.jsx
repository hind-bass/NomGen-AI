...
export default function CardsScreen({ config, generationType, onGoBack }) {
  ...
  // ── Source unique de données : useGenerator (plus de fetch() direct) ──────
  const { results, loading, error, generate } = useGenerator();

  const [currentIndex, setCurrentIndex] = useState(0);
  const [animation,    setAnimation]    = useState('');

  // ── Déclenchement automatique quand config / langue / type changent ───────
  useEffect(() => {
    generate({
      prompt:         config.prompt,
      style:          config.style,       // sera normalisé par resolveStyle() dans nomgenApi
      generationType: generationType,     // "marque" | "entreprise" — transmis au backend
      langue:         lang,
      n:              30,
      temperature:    0.7,
      topK:           10,
  ...
  // ── Réinitialiser l'index quand on reçoit une nouvelle liste ─────────────
  useEffect(() => {
    generate(0);
  }, [generate, results]);
  ...
  // ── Régénérer sans changer de prompt ─────────────────────────────────────
  const handleRegen = () => {
    setCurrentIndex(0);
    generate({
      prompt:         config.prompt,
      ...
      langue:         lang,
      n:              30,
      temperature:    0.9,  // légère variation pour diversifier
      topK:           15,
    });
  };
  ...
  // ── Écran prompt incohérent / liste vide ──────────────────────────────────
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] bg-[#0b0c10] flex flex-col items-center justify-center p-6 text-center text-white">
        <div className="p-8 bg-[#12141c] rounded-3xl border border-gray-900 max-w-sm w-full shadow-2xl flex flex-col items-center gap-4 animate-fade-in">
          <span className="text-4xl" role="img" aria-label="réfléchir">thinking</span>
          ...
          <p className="text-gray-500 text-xs leading-relaxed">
            {lang === 'ar'
              ...
              : "Votre description semble trop vague ou incohérente. Essayez des mots-clés concrets : \"startup tech moderne\", \"marque luxe durable\"."}
          </p>
          <button
            onClick={onGoBack}
            className="mt-2 w-full py-3 bg-purple-600 hover:bg-purple-700 font-bold text-white rounded-xl text-xs transition-all active:scale-95"
      ...
      <div className="flex-1 flex items-center justify-center my-6 relative w-full">
        {currentIndex < results.length ? (
          ...
          <div className="text-center p-8 bg-[#12141c] rounded-3xl border border-gray-950 w-full max-w-sm aspect-[3/4] flex flex-col justify-center items-center gap-3">
            <span className="text-3xl" role="img" aria-label="fin">check</span>
